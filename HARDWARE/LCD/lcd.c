...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(TFT_LCD->LCD_RAM)r=0;							//dummy Read	   
	opt_delay(2);	  
 	r=TFT_LCD->LCD_RAM;  		  						//实际坐标颜色
 	if(lcddev.id==0X9341)//9341要分2次读出
 	{
		...
		g<<=8;
	}else if(lcddev.id==0X6804)r=TFT_LCD->LCD_RAM;//6804第二次读取的才是真实值
...
{ 										  
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOF|RCC_APB2Periph_GPIOG|RCC_APB2Periph_AFIO,ENABLE);//使能PORTB,D,E,F,G以及AFIO复用功能时钟

 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;				 //PF10 推挽输出 背光
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)TFT_LCD->LCD_RAM=color[i*height+j];//写入数据 
	}	  
}  
//画线
//x1,y1:起点坐标
