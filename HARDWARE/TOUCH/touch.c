...
{ 	 
	...
	xdelay_us(6);//ADS7846的转换时间最长为6us
	TCLK=0; 	     	    
	xdelay_us(1);    	   
	TCLK=1;		//给1个时钟，清除BUSY	    	    
	TCLK=0; 	     	    
...
u16 TP_Read_XOY2(u8 cmd_xoy) 
{
	u16 xoy1;
 	u16 xoy2;    
	xoy1=TP_Read_XOY(cmd_xoy);
	xoy2=TP_Read_XOY(cmd_xoy);	  
...
{			    		   
	...
	//所以上拉之前,必须使能时钟.才能实现真正的上拉输出
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOF, ENABLE);	 //使能PB,PF端口时钟

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_8|GPIO_Pin_9;				 // PF689端口配置
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOF, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOF,GPIO_Pin_6|GPIO_Pin_8|GPIO_Pin_9);//上拉
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;				 // PC2 上拉输入
